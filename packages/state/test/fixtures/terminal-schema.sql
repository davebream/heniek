...
CREATE TABLE task_integration_reconciliation (
    ...
    run_id TEXT NOT NULL, task_id TEXT NOT NULL, lifecycle TEXT NOT NULL,
...
CREATE TABLE task_integration_reconciliation_observation (
    ...
    run_id TEXT NOT NULL, task_id TEXT NOT NULL, pass INTEGER NOT NULL,
    sequence INTEGER NOT NULL, repository_id TEXT NOT NULL,
    observation_json TEXT NOT NULL CHECK (json_valid(observation_json)), observed_at TEXT NOT NULL,
    UNIQUE (reconciliation_id, sequence), UNIQUE (reconciliation_id, pass, repository_id),
    FOREIGN KEY (run_id, task_id) REFERENCES task_lifecycle_projection(run_id, task_id),
...
CREATE TABLE hidden_dependency_finding (
    finding_id TEXT NOT NULL PRIMARY KEY, run_id TEXT NOT NULL REFERENCES run_projection(run_id),
    graph_id TEXT NOT NULL, graph_revision INTEGER NOT NULL, revision_sha256 TEXT NOT NULL,
    reporter_task_id TEXT NOT NULL, finding_json TEXT NOT NULL CHECK (json_valid(finding_json)),
    discovered_at TEXT NOT NULL, CHECK (graph_revision >= 1),
    CHECK (length(revision_sha256) = 64 AND revision_sha256 NOT GLOB '*[^0-9a-f]*')
) STRICT;
CREATE INDEX hidden_dependency_finding_run
ON hidden_dependency_finding(run_id, discovered_at, finding_id);
CREATE TRIGGER hidden_dependency_finding_immutable_update BEFORE UPDATE ON hidden_dependency_finding
BEGIN SELECT RAISE(ABORT, 'hidden dependency findings are immutable'); END;
CREATE TRIGGER hidden_dependency_finding_immutable_delete BEFORE DELETE ON hidden_dependency_finding
BEGIN SELECT RAISE(ABORT, 'hidden dependency findings are immutable'); END;
...
CREATE TABLE hidden_dependency_replan (
    ...
    run_id TEXT NOT NULL REFERENCES run_projection(run_id), lifecycle TEXT NOT NULL,
    proposal_json TEXT NOT NULL CHECK (json_valid(proposal_json)),
    replan_json TEXT NOT NULL CHECK (json_valid(replan_json)), revision INTEGER NOT NULL,
    created_at TEXT NOT NULL, updated_at TEXT NOT NULL,
    CHECK (lifecycle IN ('quiescing','revising','resumed','blocked')), CHECK (revision >= 1)
) STRICT;
CREATE INDEX hidden_dependency_replan_run
ON hidden_dependency_replan(run_id, created_at, replan_id);
CREATE UNIQUE INDEX hidden_dependency_replan_active ON hidden_dependency_replan(run_id)
WHERE lifecycle IN ('quiescing','revising');
CREATE TRIGGER hidden_dependency_replan_causal_update BEFORE UPDATE ON hidden_dependency_replan
...
CREATE TRIGGER hidden_dependency_replan_lifecycle_order BEFORE UPDATE ON hidden_dependency_replan
WHEN NOT (
...
CREATE TRIGGER hidden_dependency_replan_terminal BEFORE UPDATE ON hidden_dependency_replan
WHEN OLD.lifecycle IN ('resumed','blocked')
BEGIN SELECT RAISE(ABORT, 'terminal hidden dependency replan is immutable'); END;
CREATE TRIGGER hidden_dependency_replan_immutable_delete BEFORE DELETE ON hidden_dependency_replan
...
CREATE TABLE task_source_synchronization_audit (
  synchronization_id TEXT NOT NULL PRIMARY KEY REFERENCES task_source_synchronization_claim(synchronization_id),
  idempotency_key TEXT NOT NULL UNIQUE,
  outcome TEXT NOT NULL,
