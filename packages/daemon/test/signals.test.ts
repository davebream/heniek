...
describe("real out-of-process signal handling (plan Task 6 Steps 3b, 4)", () => {
  it(
    "drain race (Step 3b): a concurrent in-process second start reads a live `serving` probe and concedes exit 10, " +
      "touching neither file; a second SIGTERM then forces immediate exit without completing the graceful drain",
    async () => {
  ...
  it(
    "connectable implies fully recovered: a real authenticated daemon.status call the instant readiness lands observes " +
      "lifecycleState 'serving' and a populated reconciliation block",
    async () => {
      home = await createTempDaemonHome();
      const paths = home.home.paths;
      ...
      const status = await fetchDaemonStatusOverSocket(paths.daemonSocketFile, paths.secretsDirectory);
