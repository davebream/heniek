...
The user's real workflow uses one product-level design and plan across ten repositories, and one coherent plan task may modify several.
...
Kombajn used separate stages, but cold-start cost was a major failure.  
...
2. Treat the Kombajn extraction charter as a guard against accidental porting.
