...
**Audience.** v1: one expert technical user (Polish-speaking, English-working). Later: the global open-source developer community. The name must work in English first, must not embarrass itself in Polish, and benefits from — but does not require — continuity with the predecessor system's naming culture (*Kombajn*, Polish for combine harvester: a concrete, slightly wry, working-machine name rather than an abstract SaaS name).
...
**Soft targets:** semantic resonance with *conducting many engines*, *durability/persistence*, *waves/cadence*, *handoff/continuation*, *provenance*; a touch of the Kombajn lineage's concrete-machine humor; sound symbolism appropriate to "precise, dependable control" (see §4).
...
| Agriculture / machinery (Kombajn lineage) | F7, humor, heritage | Medium | Low, but semantics of F1–F5 weak |
...
- **kapela** — PL "band"; CZ/SK identical; ES/PT proximity to *capela/capilla* "chapel" is harmless. In PL, *kapela* is warm/folksy — consistent with Kombajn's register.
...
**osnowa** scores 5 on semantics and distinctiveness through a rare triple meaning in one Polish word: (1) the **warp** — the parallel threads held under tension that everything else is woven across (the worktrees/waves); (2) the figurative **framework** on which a story is built; (3) in surveying, *osnowa geodezyjna* — literally a **control network**: the fixed, durable reference points every measurement is anchored to. "A durable local control plane" translates itself. It also continues the Kombajn tradition of naming serious machinery in plain Polish. Its cost is usability abroad: three syllables, /os-NO-va/, and confusability with the Russian *Osnova* CMS.
...
**Heritage alternative: `osnowa`** — if the Kombajn lineage and maximal ownability outweigh international ease.
...
|---|---|---|---|---|
| **jazda** | "Jazda!" — go! let's ride! | Ignition | 5 letters; PL /ˈjaz.da/, EN reads "JAZZ-da" — the jazz-ensemble association is an accidental multi-agent fit; strongest Kombajn lineage (what you shout when the machine rolls); `jazdad` daemon weak → use `jazda-daemon` | **PASS (provisional)** — no dev-tool hits found |
...
Scores: **Public 3.73** (Sem 3 — names a real core interaction; Dist 5; Usab 3.5; X-ling 3 — EN reading genuinely broken: correct /ɕup/ "shoop", foreigners produce "sigh-up"/"see-up"; NS 4.5 ✓prov; Ext 3). **Personal ≈4.6** (Moment 4.5 — the toss; Dist 5; Usab 5 — fastest honest typing on the board; Register 5 — maximum Kombajn-core, nursery-adjacent charm; NS 4.5; Ext 3).
...
|---|---|---|---|---|
| hopla | "Hop là!" (FR/Alsatian/DE) + PL idiom "mieć hopla" = to be obsessed — self-aware joke naming the user's automation obsession; universally stable pronunciation | Unverified; adjacency: "hoopla" (EN word + major US library-streaming app, one letter away) | **~3.6** | **~3.8** |
...
| **krasnal** | Heritage | Poland's gnomes, esp. Wrocław's bronze dwarfs: hundreds of tiny workers scattered across a city, each mid-task — a codebase full of agents, self-illustrating docs | Unverified | ≈4.3 |
...
| **kombinat** | Bloodline | Kombajn's heir: harvester → industrial complex; the verb *kombinować* (creative scheming) — "Kombinat kombinuje" = the plant is scheming | Unverified | ≈4.2 |
