...
heniek config edit
heniek config resolved --codebase company-platform
```
...
Kombajn and TAKT are complementary development-time references. Neither is a runtime dependency or authoritative architecture.
...
|---|---|
| Kombajn | Cognitive workflow contracts, evidence discipline, and critic/reviewer/verifier roles |
...
The repository treats `davebream/kombajn` and `nrslib/takt` as prior art and
development-time references. Concepts have no architectural authority unless
explicitly accepted below. Claudexor is a replaceable execution backend reached
only through its versioned control API.
...
Before implementing a subsystem derived from Kombajn or TAKT:
...
- TAKT's workflow schema, transition engine, session persistence, findings,
  doctor, and subworkflow sources at the repository pin recorded in the
  dependency manifest.
