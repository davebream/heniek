...
**Supersedes:** Product Specification v0.1 without narrowing any v1 commitment or acceptance criterion  
...
`heniek` is the fixed binary and daemon name.
...
- CLI, MCP, and TUI use authenticated JSON-RPC 2.0 over a Unix domain socket.
- The local web UI binds to `127.0.0.1`, uses an authenticated session, and receives live updates over Server-Sent Events.
...
### 6.4 Initial implementation defaults

The first implementation uses TypeBox with Ajv for versioned contracts, the
built-in Node SQLite module for canonical state, Hono for authenticated local
HTTP, React with Vite for the dashboard, and Ink for the operational TUI.

These are implementation defaults, not product commitments. A spike may replace
one only through a versioned ADR that includes compatibility evidence against
the public contracts and acceptance criteria.
...
1. use `HENIEK_HOME` when set to a non-empty absolute path;
2. on Linux, use the applicable XDG locations when configured;
3. otherwise use the current user's `.heniek` directory.
...
Repositories are not required to contain `.heniek`, generated manifests, sentinels, or run history.
...
```bash
heniek profile create opus-planner
heniek pipeline validate careful-epic
heniek config edit
heniek config resolved --codebase acme-platform
```
...
v1 does not expose a fine-grained permission-policy DSL. Agent workers default to non-interactive unrestricted execution in their assigned workspace, following the documented workspace isolation policy.
...
```bash
heniek doctor
heniek engine install codex
heniek engine login claude --account claude-main
heniek engine pin cursor <version>
```
...
repositories:
  ...
  web:
    path: /workspace/acme/web
  admin:
    path: /workspace/acme/admin
  shared-client:
    path: /workspace/acme/shared-client
```
...
instructions:
  ...
  orchestrator:
    - ~/.heniek/codebases/<id>/instructions.md
```
...
```text
~/.heniek/workspaces/ws_01.../checkouts/
...
The custom daemon talks only to Claudexor's versioned `/v2` control API through an anti-corruption adapter. It does not import internal Claudexor packages.
...
```text
~/.heniek/runtimes/claudexor/<pinned-version>/
```
...
```bash
heniek export workspace <id>
heniek import <bundle>
```
...
```bash
heniek backup create
heniek backup restore
```
...
The internal predecessor orchestrator and TAKT are complementary development-time references. Neither is a runtime dependency or authoritative architecture.

Heniek owns all product and domain decisions. References contribute evidence and
mechanics only through explicit, provider-neutral contracts:

| Reference | Heniek borrows |
|---|---|
| Internal predecessor | Cognitive workflow contracts, evidence discipline, and critic/reviewer/verifier roles |
| TAKT | YAML and state-machine mechanics, sessions, transitions, findings, doctor, and subworkflows |
| Claudexor | Provider processes, accounts, sessions, interactions, and execution |
| Heniek | Codebases, multi-root workspaces, task waves, integration, and delivery |

The repository treats an internal predecessor orchestrator and `nrslib/takt` as
prior art and development-time references. Concepts have no architectural
authority unless explicitly accepted below. Private predecessor source locations
and revision pins are not recorded in this public repository. Claudexor is a
replaceable execution backend reached only through its versioned control API.
...
| Repository-local predecessor runtime state | Global application home |
...
Before implementing a subsystem derived from the predecessor or TAKT:
...
Development should consult the privately maintained predecessor architecture,
workflow, schema, scheduling, reconciliation, and audit records relevant to the
subsystem being implemented, plus TAKT's workflow schema, transition engine,
session persistence, findings, doctor, and subworkflow sources at the repository
pin recorded in the dependency manifest.
...
### Over-engineering from the predecessor
...
- the predecessor extraction charter is accepted;
...
Naming is resolved: Heniek is the fixed product, binary, and daemon name.

The v1 product commitments in this document are fixed. Technical spikes may
revise an implementation default through a versioned ADR and compatibility
tests, but may not narrow v1 scope or remove an acceptance criterion.
