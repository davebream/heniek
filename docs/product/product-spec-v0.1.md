...
tool config edit
tool config resolved --codebase acme-platform
```
...
## 32. Development-time predecessor extraction charter

Heniek draws selected workflow principles from an internal predecessor orchestrator. The predecessor is not a runtime dependency and has no architectural authority unless a concept is explicitly accepted below.
...
Before implementing a subsystem derived from the predecessor:
...
subsystem being implemented. Private source locations and revision pins are not
